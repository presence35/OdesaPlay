...
export default function SalesTool({ lang }: { lang: Language }) {
  ...
  return (
    <div className="text-[var(--text-primary)] font-sans max-w-6xl mx-auto space-y-8">
      {/* Hero */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center pt-4 pb-2">
        <h1 className="text-3xl sm:text-4xl font-black italic uppercase tracking-tight text-[var(--text-primary)]">
          {t.restaurantName}
        </h1>
      </motion.div>
      ...
      <div className="flex justify-center gap-2">
        {timePills.map(p => (
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider transition-colors ${timePeriod === p.key ? 'bg-[var(--btn-primary-bg)] text-[var(--btn-primary-text)]' : 'bg-[var(--bg-elevated)] text-[var(--text-subtle)] hover:text-[var(--text-primary)]'}`}
          >
            {p.label}
          </button>
        ))}
        ...
        <div className="grid grid-cols-2 gap-4">
          {KPI.map((kpi, i) => (
            ...
            >
              ...
              <div className="text-xs uppercase font-bold text-[var(--text-muted)] tracking-widest">{kpi.label}</div>
            </motion.div>
          ))}
        </div>
      </motion.div>
      ...
      <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}
        className="bg-[var(--bg-secondary)] border border-[var(--border-strong)] rounded-3xl p-6 sm:p-8">
        <div className="flex items-center gap-3 mb-2">
          <MapPin className="text-blue-400 w-5 h-5" />
          <h2 className="text-xl font-bold text-[var(--text-primary)] uppercase tracking-wider">{t.customerSources}</h2>
        </div>
        <p className="text-sm text-[var(--text-muted)] mb-6">{t.sourcesSubtitle}</p>
          ...
          <div className="space-y-3">
            {scaledSources.map((s, i) => (
              <div key={s.name} className="flex items-center gap-3">
                ...
                <span className="flex-1 text-sm text-[var(--text-muted)]">{sourceLabel(s.name)}</span>
                <span className="text-sm font-bold text-[var(--text-primary)]">{s.value}</span>
                <span className="text-xs text-[var(--text-muted)] w-10 text-right">{sourceTotal > 0 ? ((s.value / sourceTotal) * 100).toFixed(0) : 0}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-xl font-bold text-[var(--text-primary)] uppercase tracking-wider">{t.conversionFunnel}</h2>
        </div>
        <p className="text-sm text-[var(--text-muted)] mb-6">{t.funnelSubtitle}</p>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
            ...
            {scaledFunnel.map((s, i) => (
              <motion.div key={s.stage} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.4 + i * 0.08 }}
                ...
                <div className="text-right shrink-0 w-24">
                  <div className="text-sm font-bold text-[var(--text-primary)]">{s.value.toLocaleString()}</div>
                  <div className="text-xs text-[var(--text-muted)]">{funnelLabel(s.stage)}</div>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-xl font-bold text-[var(--text-primary)] uppercase tracking-wider">{t.mostPlayedGames}</h2>
          </div>
          <p className="text-sm text-[var(--text-muted)] mb-6">{t.gamesSubtitle}</p>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.5 }}
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-xl font-bold text-[var(--text-primary)] uppercase tracking-wider">{t.crossRestaurantTraffic}</h2>
          </div>
          <p className="text-sm text-[var(--text-muted)] mb-6">{t.crossSubtitle}</p>
          <div className="space-y-3">
            {scaledCross.map((item, i) => (
              <motion.div key={item.restaurant} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.6 + i * 0.08 }}
                className="flex items-center gap-4 p-4 bg-[var(--bg-elevated)]/30 rounded-2xl border border-[var(--border-strong)] hover:border-cyan-500/30 transition-colors group">
                ...
                <div className="flex-1">
                  <p className="text-sm font-bold text-[var(--text-primary)]">{item.restaurant}</p>
                  <p className="text-xs text-[var(--text-subtle)]">{(item.guests === 1 ? t.sentXGuest : t.sentXGuests).replace('{n}', String(item.guests)).replace('{restaurant}', item.restaurant)}</p>
                </div>
      ...
      <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.6 }}
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <h2 className="text-xl font-bold text-[var(--text-primary)] uppercase tracking-wider">{t.weeklyGrowth}</h2>
        </div>
        <p className="text-sm text-[var(--text-muted)] mb-6">{t.growthSubtitle}</p>
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
