  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        ...
        {subtitle && <p className="mb-8 mx-auto opacity-80 max-w-sm text-lg text-slate-300">{subtitle}</p>}
        <div className="flex gap-8 sm:gap-12 mb-10 bg-black/30 px-16 py-10 rounded-[2rem] backdrop-blur-md border border-white/10 items-center justify-center shadow-2xl">
          <div className="flex flex-col items-center">
            {imageSrc && (
              <img src={imageSrc} className="w-20 h-20 object-contain mb-2 drop-shadow-xl" alt="" />
            )}
            {children}
        ...
        <div className="flex flex-col items-center gap-3">
          <button
            onClick={onPlayAgain}
            className={`px-10 py-5 rounded-2xl font-black text-2xl transition-[transform,shadow,filter] duration-200 active:scale-95 shadow-2xl hover:scale-105 transform-gpu ${
              isWin
          ...
          {onQuit && (
            <button
              onClick={onQuit}
              className="text-red-400/60 hover:text-red-400 text-xs font-bold uppercase tracking-widest transition-colors mt-1"
            >
              {t.quit || 'Quit'}
            </button>
          )}
